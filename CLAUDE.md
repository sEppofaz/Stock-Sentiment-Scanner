...
## Fable-Analyse Trefferquote + volume_anomaly-Richtungsfix (2026-09-11)

Josef-Wunsch: „wirklich gute Ergebnisse" statt der gemessenen 31,7%/47,8% Trefferquote (siehe „Median + Trefferquote"-/„Split-bereinigte Renditeberechnung"-Abschnitte oben) – unabhängige Fable-Analyse von Score-Formel, Auswertungsmethodik und Frühsignal-Layern beauftragt (reine Analyse, kein Code-Auftrag). Vollständiger Bericht nur im Claude-Session-Log, hier nur die umgesetzte Konsequenz + offene Punkte.

- **Gefixt – `volume_anomaly` war richtungsblind:** `layer2_volume.py::run_volume_scan()` bewertete jeden Volumen-Ausschlag (egal ob Kurs stieg oder fiel) als potenzielles Kaufsignal – ein Abverkauf auf hohem Volumen (Panik, Block-Trade, Delisting-Angst) erzeugte dieselbe Gewichtung wie echtes Kaufinteresse. Passt zum bereits dokumentierten VYNE-Befund (extreme z-Scores = schlechtere Performance in der ersten Stichprobe).
  - `layer2_volume.py`: liest jetzt zusätzlich `Close` aus dem ohnehin geladenen `yf.download()`-Ergebnis (kein neuer API-Call), speichert `direction` (`"up"`/`"down"`) + `price_change_pct` in den Signal-Details. Signal wird für BEIDE Richtungen weiter gespeichert (nicht gefiltert) – `layer6_sell_signal.py` braucht den fallenden Fall für Verkaufssignale bei echten Positionen.
  - `layer4_scoring.py`: `run_scoring()` überspringt `volume_anomaly` mit `direction=="down"` bei der Gewichtsberechnung (zählt nicht mehr zum Kombi-Score); `check_instant_alerts()` verlangt für den `strong`-Flag jetzt explizit `direction=="up"`.
  - `layer6_daily_pick.py::_distinct_signal_types_recent()`: zählt `volume_anomaly` bei fallendem Kurs nicht mehr als bestätigenden Signal-Typ für die C1-Kombi-Bestätigung.
  - **Abwärtskompatibel:** Alte Signale ohne `direction`-Feld (vor diesem Fix) werden weiterhin wie bisher als bullish gezählt – altern aber binnen der jeweiligen Zeitfenster (20 Min Instant-Alerts, 7 Tage Kombi-Score) automatisch aus, kein Backfill nötig.
  - Getestet: isolierte Logik-Tests mit synthetischen Daten (Richtungserkennung, Scoring-Gewicht, Instant-Alert-strong-Flag, Distinct-Type-Zählung – alle 4 Testgruppen bestanden), Python-Syntax-Check, live deployed + `journalctl` verifiziert (Scheduler-Jobs starten sauber, `/sentiment/login` antwortet 200).
- **Nachgeholt direkt im Anschluss (Josef: „wieso nicht gleich umsetzen"):**
  - **`_suggest_adjustments()` auf Median umgestellt:** rechnete trotz des Anzeige-Fixes vom 21.08. weiterhin mit Mittelwerten – da diese Funktion tatsächlich automatisch `config.json` verändert (`_apply_suggestions`), konnte ein einzelner Datenfehler (wie ADR-019) live Schwellenwerte verzerren. Suggestion-Felder von `pos_mean`/`neg_mean` auf `pos_median`/`neg_median` umbenannt, Anzeigetext von „Ø" auf „Median".
  - **Marktbenchmark (IWM + SPY parallel):** `forward_returns`/`scan_forward_returns` haben neue Spalten `benchmark_iwm_ret_pct`/`benchmark_spy_ret_pct` (gleiche Berechnung wie die Ticker-eigene Rendite, `forward_tracker.py`/`scan_tracker.py` cachen die Benchmark-Kurse pro Start-Datum innerhalb eines Laufs). `weekly_analysis.py` liefert zusätzlich `overall_vs_iwm`/`overall_vs_spy` (Excess-Return = eigene Rendite minus Benchmark-Rendite im selben Fenster). Rückwirkender Backfill der kompletten Historie (2149 `forward_returns` + 25194 `scan_forward_returns`, 100% erfolgreich befüllt) via neuem `backfill_benchmark_returns.py` (analog `backfill_split_adjusted_returns.py`, ADR-019) – einmalig auf dem Server ausgeführt.
  - **Pseudo-Replikation behoben:** neue `_dedupe_overlapping()` in `weekly_analysis.py` behält pro Ticker nur den ersten Datenpunkt je nicht-überlappendem Horizont-Fenster (20 Tage für die Hauptauswertung, 1/5 Tage für die Zusatz-Horizonte) – verhindert, dass derselbe Ticker bei laufender Signallage mehrfach als vermeintlich unabhängiger Datenpunkt in Trefferquote/Median UND in `_suggest_adjustments()` eingeht. Angewendet in `_analyze_sentiment()`/`_analyze_early_signals()`, wirkt automatisch auf alles, was aus `rows`/`alert_rows` abgeleitet wird.
  - **1-/5-Tage-Horizont:** neue `overall_1d`/`overall_5d`-Felder (gleiche Dedup-Regel) zusätzlich zum bisherigen 20-Tage-Hauptbericht (`overall`) – nur Report-/API-Zahlen, keine PWA-Änderung in diesem Schritt (bewusster Scope-Cut).
  - **Live-Ergebnis nach Dedup + mehr gereiften Daten (2026-09-11, zum Vergleich mit den Werten vom 21.08.):** Sentiment-Scan n=408 (vorher n=60), Median **−2,95%** (vorher −13,46%), Trefferquote **38,5%** (vorher 31,7%) – **markt-relativ (vs. IWM) Trefferquote 47,3%**, deutlich näher an 50% als die absolute Zahl. Frühsignale n=379, Median −2,56%, Trefferquote 40,1%, vs. IWM 41,7%. Stützt Fables Vermutung, dass ein relevanter Teil der schlechten absoluten Zahlen am fallenden Gesamtmarkt lag, nicht nur am Signal selbst – „gut" ist es damit aber weiterhin nicht.
  - Getestet: isolierte Logik-Tests mit synthetischen Daten (Dedup-Fensterlogik, Benchmark-Excess-Berechnung, Backfill-`_compute()`-Kursausrichtung), Syntax-Checks, Backfill zuerst `--dry-run` dann live, `analyze_and_store()` für beide Systeme live gegen die echte DB verifiziert (kein Telegram-Nebeneffekt, gleiches Verifikations-Muster wie beim Split-Fix 21.08.).
- **Weiterhin offen (Josef-Entscheidung wegen laufender Kosten):** `ki_enabled` (Claude-Sentiment statt Keyword-NLP) steht standardmäßig auf `false` – ~0,36€/Tag bzw. ~11€/Monat bei 2 Scans/Tag, falls aktiviert. Als PKA-Todo erfasst, kein Alleingang.
- **Fables ehrliche Gesamteinschätzung:** Mit reinem Free-Tier-Keyword-Sentiment auf illiquiden Microcaps (50M–2Mrd$ Market Cap) ist eine konstant hohe Trefferquote strukturell unwahrscheinlich – „buzz" könnte in diesem Segment eher Kontraindikator (Pump-Peak) als Kaufsignal sein. Der Insider-Kauf-Layer (Layer 1) gilt als am besten fundiert, wird aber aktuell additiv mit schwächeren Signalen verwässert statt als primäres Filterkriterium behandelt.

## 70%-Trefferquote-Frage + Payoff-Ratio/Expectancy + Datenqualitäts-Fund (2026-09-11, Teil 2)

Direkter Anschluss: Josef – „ki-enabled kann ich selber einschalten, um wie viel werden die Ergebnisse besser? Wenn wir unter 70% sind, brauchen wir einen ganz neuen Ansatz." `ki_enabled` war nie in echtem Betrieb (nur 10 Testdatenpunkte von einem einzigen Scan am 06.07.) – keine Vorher/Nachher-Daten möglich. Zweite Fable-Analyse beauftragt: ist 70% überhaupt ein sinnvolles Ziel, welche strukturell neuen Ansätze kämen infrage?

- **Vorab-Diagnose (eigene, ohne Fable):** Trefferquote je Signal-Typ aufgeschlüsselt (dedupliziert, n=379 Frühsignal-Alerts) – nur `insider_buy` isoliert liegt über 50% (54,3% absolut/57,1% vs. IWM), `volume_anomaly` isoliert sogar unter Marktniveau (29,5% vs. IWM). Insider+Volumen kombiniert performt schlechter (35%) als Insider allein – additive Kombination verwässert eher, als sie verstärkt. 13D/13G: n=0 gereifte Daten.
- **Fables Kernaussage: 70% ist die falsche Metrik, nicht nur ein schwer erreichbares Ziel.** Selbst institutionelle Quant-Fonds erreichen bei Einzelpositionen mit festem Zeithorizont selten über 50-58% Trefferquote; Trendfolge-Systeme laufen oft profitabel mit nur 35-45%, weil Gewinner deutlich größer sind als Verlierer. Die eigentlich fehlende Kennzahl: Payoff-Ratio (Ø-Gewinner/Ø-Verlierer) + Expectancy, nicht nur Trefferquote.
- **Payoff-Ratio/Expectancy berechnet – dabei zweiter, unabhängiger Datenqualitäts-Fund:** Erster naiver Durchlauf zeigte absurde Zahlen (Payoff-Ratio 11, Expectancy +58%/Position) – Ursache: 11 von 408 Sentiment-Picks (≈3%) hatten Renditen von +200% bis +8.667%, die beim Gegenchecken mit einem frischen yfinance-Fetch NICHT reproduzierbar waren (z.B. SXTC gespeichert +4.747,83%, frisch berechnet −39,4%; NXTT gespeichert +8.666,67%, frisch −12,3%). **Root Cause: Yahoo Finance korrigiert historische Tages-Closes nachträglich still** (kein Split-Problem, ADR-019 war korrekt) – `forward_tracker.py`/`scan_tracker.py` füllten `ret_pct` bisher genau einmal und prüften nie erneut nach.
  - **Fix:** neues `return_reverify.py` – re-fetcht gruppiert nach (Ticker, Datum), überschreibt `ret_pct` nur bei grober Abweichung (≥30 Prozentpunkte, verhindert Flip-Flop bei normalem Kursrauschen). `reverify_recent()` (14-Tage-Fenster) läuft ab jetzt täglich im bestehenden Cleanup-Job (`app.py::_do_cleanup`, 03:00 UTC). Bewusst KEIN neues Schema/Flag – `ret_pct` wird direkt korrigiert, bestehende `weekly_analysis.py`-Queries profitieren ohne Änderung.
  - **Einmaliger Full-Backfill** (`backfill_reverify_returns.py`, ~30 Min Laufzeit wegen unbatchter Einzel-Ticker-Downloads – bewusst nicht optimiert, da Einmal-Lauf): 15 `forward_returns` + 73 `scan_forward_returns` von 27.343 geprüften Zeilen korrigiert.
  - **Bekannte Grenze, kein Bug:** Ein Fall (`UCAR`) wurde durch einen sehr frischen, von Yahoo noch nicht rückwirkend bereinigten Reverse-Split (2026-09-10, Kurssprung ×15,7 an einem Tag) auf einen selbst noch falschen Wert „korrigiert" (h=1/h=5, noch nicht h=20). Behebt sich durch den täglichen `reverify_recent()`-Lauf von selbst, sobald Yahoo den Split verarbeitet hat.
  - Getestet: isolierte Logik-Tests mit synthetischer In-Memory-SQLite-DB + gefälschtem `fetch_closes()` (großer Abweichung wird korrigiert, kleine bleibt unangetastet), Syntax-Checks, live deployed + Backfill-Ergebnis gegen echte DB verifiziert.
- **Robustes Payoff-Ratio-Ergebnis (nach Reverify-Fix, Median-Gewinner/-Verlierer statt Mittelwert):**

  | Gruppe | n | Trefferquote | Payoff-Ratio | Expectancy/Position |
  |---|---|---|---|---|
  | Sentiment-Scan gesamt | 408 | 36,8% | 1,08 | **−3,92%** |
  | Frühsignale gesamt | 379 | 38,8% | 1,05 | **−3,79%** |
  | nur `insider_buy` | 35 | 54,3% | 1,30 | **+1,69%** |
  | nur `volume_anomaly` | 132 | 37,9% | 1,06 | **−7,43%** |

  **Fazit: Das System hat aktuell keine nachweisbare positive Expectancy außer im zu kleinen Insider-Kauf-Segment (n=35, 95%-CI ±16pp laut Fable – statistisch nicht von den anderen Gruppen unterscheidbar).** Bestätigt Fables Linie noch deutlicher als die ersten (unbereinigten) Zahlen vermuten ließen.
- **Fables priorisierte Empfehlungen (sofort/kostenlos zuerst):** (1) Insider-Kaufgröße relativ zur Marktkapitalisierung normieren statt fixer USD-Schwellen (höchster Hebel, MarketCap wird bereits abgerufen) – **noch offen**. (2) Additive Kombi-Logik als primäres Kaufkriterium zurückstufen, `volume_anomaly`/Buzz+Volumen nicht mehr als eigenständigen Kaufanlass werten – **noch offen**. (3) 13F-Änderungen (bestehende EDGAR-Infrastruktur) + Quiver Quant (Congressional Trading, kostenlos) als neue, mechanistisch konsistente Layer – **noch offen**. (4) Exit-/Risikomanagement (Trailing-Stop-Parameter) verschärfen statt weiter an Einstiegssignalen drehen – laut Fable der Hebel mit bester Aussicht ohne neue Unsicherheit – **noch offen**. (5) Insider-Only-Bucket weiter beobachten bis n>100 (9-12+ Monate bei aktuellem Form-4-Tempo) vor jeder Kapital-/Architekturentscheidung. Alle fünf als PKA-Todos erfasst, keine Umsetzung heute (Josef für den Tag nicht mehr erreichbar).
- **Übergeordnete Konsequenz:** 70%-Trefferquote-Ziel explizit aufgeben, durch Expectancy-/Payoff-Ratio-Ziel ersetzen. Realistischer Zielkorridor laut Fable: 45-55% Trefferquote kombiniert mit nachweislich positiver Expectancy – noch nicht erreicht, aber jetzt ehrlich messbar.

## Nur noch insider_buy + large_holder als Kaufsignal (2026-09-12, v1.30, ADR-022)

Direkter Anschluss an die 70%-Trefferquote-Frage: Josef entschied nach kurzer Diskussion „radikal vereinfachen" (mit drei Präzisierungen per Rückfrage: Scans laufen weiter/kein API-Kosten-Unterschied, 13D/13G bleibt zulässig, PWA zeigt deaktivierte Signale weiter mit Kennzeichnung). Auf die Frage „App ändern oder neu bauen?" klare Empfehlung für Modifikation (bestehende Datenpipeline + `signals.db`-Historie sind der eigentliche Wert, tatsächlicher Änderungsumfang klein) – Josef bestätigt.

- **Config-getrieben (reversibel), nicht hart verdrahtet:** `early_signals.actionable_types` (Default `["insider_buy","large_holder"]`) steuert in `layer4_scoring.py` (`run_scoring()`/`check_instant_alerts()`), welche `signal_type`-Werte überhaupt Alert-relevant sind. `daily_pick.sentiment_scan_actionable` (Default `false`) schaltet den Sentiment-Scan-Snapshot-Pfad (C2/C3) in `layer6_daily_pick.py` ab – Block bewusst NICHT entfernt, nur ausgeklammert (`if sentiment_scan_actionable:`). Details/Alternativen: ADR-022.
- **Cross-Signal wird dadurch automatisch inert statt kaputt:** `_has_cross_signal()` braucht zwingend einen Snapshot-Kandidaten – ohne den C2/C3-Block kann `cross` nie mehr `True` werden. `_SOURCE_PRIORITY`/`_has_cross_signal()` bleiben unverändert im Code stehen (nicht als toter Code entfernen – werden bei `sentiment_scan_actionable=true` wieder aktiv).
- **`_distinct_signal_types_recent()`** (layer6_daily_pick.py) bekommt einen neuen Pflicht-Parameter `actionable: set[str]` – zählt nur noch aktionable Typen für die C1-Kombi-Bestätigung, sonst könnte z.B. `volume_anomaly`+`buzz_accel` weiterhin als „≥2 Typen" durchgehen, obwohl beide separat kein Gewicht mehr beitragen.
- **V1-Veto (volume_anomaly-Plausibilitätsgrenze `max_volume_z`) bewusst unverändert:** bleibt sinnvoll als Veto gegen unplausible Ausreißer, unabhängig davon ob volume_anomaly ein eigenständiges Kaufsignal ist.
- **Keine Server-Config-Migration nötig, live verifiziert:** Server-`config.json` hatte die neuen Keys nach dem Deploy erwartungsgemäß nicht (gitignored, `git pull` überschreibt sie nicht) – `.get(key, default)`-Fallback griff ohne manuellen Eingriff, per SSH-Check bestätigt.
- **Nebenbei gefundener + gefixter Bug:** `saveConfig()` in der PWA baute den `early_signals`-Block ohne `..._cfg.early_signals`-Spread neu auf (Regression des bereits 2026-07-07 gefixten Bug-Typs, siehe damaliger „Kritischer Bugfix") – ein server-seitig gesetztes `actionable_types` wäre beim nächsten „Speichern" im Einstellungen-Tab stillschweigend verworfen worden. **Regel bekräftigt:** JEDES Config-Unterobjekt in `saveConfig()` muss `..._cfg.<key>` spreaden, nicht nur einzelne (weekly_analysis/trailing_stop taten das bereits korrekt, early_signals nicht).
- **PWA (v1.29 → v1.30):** `_actionableTypes` (global, aus `cfg.early_signals.actionable_types` abgeleitet, nicht hart kodiert – vermeidet Text/Config-Drift) steuert Dimmen + „⏸ nicht mehr aktionabel"-Badge für `volume_anomaly`/`buzz_accel` in Filter-Chips (`renderEsFilterChips()`) und Signal-Feed-Items (`renderEsSignals()`). `loadConfig()` jetzt zusätzlich im App-Init aufgerufen (vorher nur bei Tab-Wechsel auf „Einstellungen"), damit `_actionableTypes` vor dem ersten Besuch des Früh-Tabs gesetzt ist. **Pitfall vermieden:** Badge-Text „nicht mehr aktionabel" NICHT in das bestehende `nowrap`-Label eingebettet (hätte das 2026-07-08-Pitfall-Muster reproduziert), stattdessen als eigene Zeile unterhalb des Signal-Items. Info-Sheet-Texte an mehreren Stellen aktualisiert (Frühsignale-Intro, Volumen-Anomalie/Buzz-Beschleunigung, „Wann ist das ein Kauf-Hinweis?", Tages-Pick-Absatz, Performance-Analyse-Absatz) – sonst hätten sie nach dem Backend-Fix nicht mehr gestimmt.
...
## Layer 7: Value/Quality-Screening auf fundamentale Unterbewertung (2026-09-12)

Josefs Anschlussfrage nach Fables Payoff-Analyse (siehe oben): Ideen für automatisierte Suche nach fundamental unterbewerteten, aussichtsreichen Aktien. Bewusst als eigenständiger Layer im bestehenden Projekt (nicht neue App) – Josef-Entscheidung.

- **`layer7_value.py`**, neuer Signaltyp `"value"` in der generischen `signals`-Tabelle (kein Schema-Change nötig). Rang-basiertes Scoring (Value-Rang KGV/P-B/EV-FCF + Quality-Rang ROE/Current-Ratio/Debt-Equity/Umsatzwachstum, je 0-100 Perzentil über die gefilterte Kandidatenmenge, 50/50 kombiniert) statt additiver Bonuspunkte wie beim bestehenden KGV-Bonus in `scanner._calc_score()`.
- **Finnhub-Feldverfügbarkeit live verifiziert (`/stock/metric`, AAPL/F/KSS/PLUG/GPRO):** `roicTTM` existiert NICHT im Free Tier → `roeTTM` als primäre Profitabilitäts-Kennzahl (nicht `roiTTM`, das ist methodisch ROI statt ROIC und wird bewusst nicht verwendet). `evEbitda` existiert ebenfalls NICHT → `currentEv/freeCashFlowTTM` (EV/FCF) als Ersatz, methodisch sogar robuster (Capex eingerechnet, schwerer bilanziell zu schönen). `enterpriseValue` (nicht `ev`), `pbAnnual`, `totalDebt/totalEquityAnnual`, `currentRatioAnnual`, `netProfitMarginTTM`, `revenueGrowthTTMYoy` bestätigt durchgehend befüllt (auch bei negativen Werten), `peNormalizedAnnual` `None` bei Verlust-Firmen (erwartbar).
- **Kein voller Piotroski-F-Score/Altman-Z-Score möglich:** Finnhub `/stock/metric` liefert nur den aktuellen Wert, keine Zeitreihe für YoY-Trend-Kriterien. `_f_score_reduced()` gibt deshalb `(erreichte_punkte, verfügbare_kriterien)` statt eines vollen 0-9-Scores zurück – reine Zusatzinfo in `details_json`, nicht Teil des Rankings. `_distress_veto()` (Current Ratio < 1 UND negative ROE) ersetzt einen vorgetäuschten Altman-Z-Score als grobes Ausschlusskriterium.
- **Bewusst KEIN Combo-Scoring mit Sentiment/Frühsignalen:** Direkte Lehre aus ADR-022 (gleicher Tag) – additive Kombination unbewiesener Signaltypen hatte dort die Expectancy verschlechtert statt verbessert. Der Value-Layer bleibt eigenständig (eigener Signaltyp, eigener Tab/Endpoint geplant, eigener Langzeit-Tracker), bis eigene Evidenz über Monate zeigt, ob er überhaupt funktioniert.
- **Scheduler bewusst NICHT im werktäglichen 15-Min-Raster:** Fundamentaldaten ändern sich nicht untertägig, und ein Scan über alle ~4.700 Ticker braucht beim gemeinsamen 55-Calls/Min-Throttle (`scanner._throttle()`) selbst ~85 Minuten. Eigener Slot **Samstag 06:00 UTC** (`value_scan`-Job in `_reschedule()`) – dort läuft sonst kein Finnhub-Job (alle bestehenden sind `mon-fri`), kein Throttle-Konflikt.
- **Kill-Switch `value_layer.enabled`** (Default `false` in `config.default.json`), analog `early_signals.enabled` – live verifiziert: Service-Neustart nach Deploy zeigt `value_scan` erwartungsgemäß NICHT in der APScheduler-Jobliste, obwohl der Server-`config.json` das neue Objekt (gitignored) noch fehlt (`.get("value_layer", {}).get("enabled", False)`-Fallback greift, wie schon bei ADR-022 verifiziert).
- **Manueller Trigger `/sentiment/api/value/run`** (POST) läuft im Background-Thread (wie `/api/scan`, sonst Request-Timeout bei ~85 Min) und ruft `run_value_scan()` direkt auf – umgeht bewusst den `enabled`-Check (analog `run_daily_pick_manual`), damit man ohne Config-Änderung/Warten auf Samstag testen kann. Eigener Status-Dict `VALUE_SCAN_STATUS` (analog `scanner.SCAN_STATUS`) verhindert Überlappung zweier Läufe und liefert `/sentiment/api/value/status` Fortschritt.
...
- **Noch offen (nicht Teil dieser Phase):** PWA-Tab „Value" + Endpoint-Anzeige, Langzeit-Tracker (`value_tracker.py`, Horizonte 60/120/250 Handelstage statt der bestehenden 1/5/20 – Value-Picks brauchen Monate, nicht Tage), echter Vollscan über alle ~4.700 Ticker (bisher nur 10-Ticker-Smoke-Test).
