...
- **Bewusst noch NICHT umgesetzt (größerer Umfang, eigene Session):** Marktbenchmark für Trefferquote (aktuell absolut, nicht relativ zum Russell 2000/IWM), Pseudo-Replikation in `_analyze_sentiment()`/`_analyze_early_signals()` (überlappende 20-Tage-Fenster desselben Tickers zählen mehrfach als „unabhängige" Datenpunkte), `_suggest_adjustments()` nutzt weiterhin Mittelwerte statt Median (inkonsistent zum Median-Fix vom 21.08. bei der reinen Anzeige), nur 20-Tage-Horizont wird ausgewertet (1-/5-Tage-Daten liegen bereits vor, ungenutzt), `ki_enabled` (Claude-Sentiment statt Keyword-NLP) steht weiterhin standardmäßig auf `false`. Als PKA-Todos erfasst.
