...
- **`PATCH /api/portfolio/<ticker>/convert`** ("Zu echter Position machen", Josef-Wunsch): wandelt eine 👁-Beobachtung (`watch:true`, 1 Test-Aktie) in eine echte Position um – setzt `watch=false` + die tatsächlichen `shares`/`buy_price`/optional `buy_date`. 404 wenn Ticker nicht gefunden, 400 wenn bereits `watch:false`. PWA: Button auf jeder 👁-Karte, `prompt()`-Dialoge für Anzahl/Kurs/Datum (kein neues Formular).
